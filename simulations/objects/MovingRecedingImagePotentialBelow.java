...
/** 
 * MovingRecedingImagePotentialBelow.  This BaseObject calculates the non-relativistic magnetic fields below a thin
 * conducting plane due to a moving magnetic monopole a distance H above the plane, following the analogous formula
...
public class MovingRecedingImagePotentialBelow extends BaseObject {
  /** mhu naught over 4 pi, the overall multiplicative constant*/
  static protected double Bfactor = Constants.u0/(4.0*Math.PI);
  ...
  public MovingRecedingImagePotentialBelow(double H, double q0, double v0, double v,  double zplane, double offset){
	  this.offset = offset;  
	    this.q0 = q0;
	    this.H = H;
	    this.v0 = v0;
  ...
  public Vec3 Bfield(Vec3 x, Vec3 B) {
    /* calculates B by numberically taking gradient of the potential analogous to Saslow eq 65 */
   ...
   * @param x the position of the observer
   * @param E the electric field at the observer's position if the monopope is at (0,0,H) (calculated) 
   * @return zero  */  
  ...
  public double Potential(Vec3 x, double potential) {
     ...
     potential = Bfactor*q0*(v/fact1)*(fact2/fact4+fact3/fact5);
	 return potential;
	}
