 ...
 * This version of the program calculates the 
 * magnetic fields using line dipoles and monopoles.  
 ...
 * @version 1.0
 */
public class MovingReceedingImageExperiment extends BaseExperiment {
  ...
  public MovingReceedingImageExperiment(double H, double q0, double v0, double v,  double zplane, double offset){
  ...
  public void ConstructEMSource(){
    // public MagneticMonopole(double q, Vec3 p, Vec3 v, int image, double radius)
	  //(Vec3 mdipole, Vec3 tline, Vec3 p, Vec3 v, int image)
    ...
    lineplus = new Line3DMagneticDipoles(new Vec3(mlengthx, 0., mlengthz),new Vec3(-v, 0., -v0), new Vec3(0., 0., dbelow), new Vec3(0., 0., 0.),zplane,-1);
    lineminus = new Line3DMagneticDipoles(new Vec3(-mlengthx, 0., mlengthz),new Vec3(-v, 0., v0), new Vec3(0., 0., H), new Vec3(0., 0., 0.),zplane,1);
    monoplus = new MagneticMonopole(qimage, new Vec3(0., 0., dbelow),new Vec3(0., 0., 0.),zplane,-1, 0.);
    monominus = new MagneticMonopole(-qimage, new Vec3(0., 0., H),new Vec3(0., 0., 0.),zplane, 1, 0.);
    collection = new EMCollection();
    
    collection.Add(mono);
    collection.Add(monoplus);
    collection.Add(monominus);
    collection.Add(lineplus);
    collection.Add(lineminus);
  ...
  public void Evolve(double dt, double maxStep){
    ...
    if (vcross <= 0. ) vcross = 0.000;
    if ( t/dt >= 30.9 ) v = vcross;
    if ( t/dt >= 30.9 )  v = vcross;
    ...
    mlengthx = mlength*(v0/Math.sqrt(v*v+v0*v0));
    mlengthz = -mlength*(v/Math.sqrt(v*v+v0*v0));
    System.out.println(mlength + " FGHJKL " + mlengthx + " " + mlengthz + " q0  " + q0 + "v0 " + v0 );
    lineplus.mdipole = new Vec3(mlengthx,0.,mlengthz);
    lineplus.tline = new Vec3(-v,0.,-v0);
    monoplus.q=qimage;
